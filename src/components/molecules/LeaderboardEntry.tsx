...
export const LeaderboardEntry: React.FC<LeaderboardEntryProps> = ({ entry, rank }) => {
  ...
  return (
    <div
      className={`leaderboard-entry ${isPodium ? 'leaderboard-entry--podium' : ''} ${isPitting ? 'leaderboard-entry--pitting' : ''}`}
    >
      {/* Position */}
      ...
      <div className="leaderboard-entry__driver">
        <div className="leaderboard-entry__name">{entry.driver.lastName}</div>
        <div className="leaderboard-entry__team">{entry.team}</div>
      </div>

      {/* Gap to Leader */}
      <div className="leaderboard-entry__gap">
        <span
          className={`leaderboard-entry__gap-value ${entry.gapToLeader === 'LEADER' ? 'leaderboard-entry__gap-value--leader' : ''}`}
        >
          {entry.gapToLeader}
        </span>
      </div>
      ...
      {/* Interval */}
      <div className="leaderboard-entry__interval">{entry.interval}</div>
      ...
      {entry.pitStatus !== 'GREEN' && (
        <div
          className={`leaderboard-entry__pit-status leaderboard-entry__pit-status--${entry.pitStatus.toLowerCase()}`}
        >
