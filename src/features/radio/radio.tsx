...
export const TeamRadio: React.FC = () => {
  ...
  const [driverNumber, setDriverNumber] = useState('1'); // Default to Driver #1 (e.g. Verstappen)
  
  // 1. Fetch recent sessions to acquire an active session key
  const { data: sessions, isLoading: sessionsLoading } = useGetSessionsQuery({ year: '2024', session_name: 'Race' });
  
  const activeSessionKey = sessions?.[0]?.session_key || '9158'; // Fallback to a valid demo session

  // 2. Fetch specific team audio segments
  ...
  return (
    <div className="f1-team-radio rounded-xl border border-slate-800 bg-slate-900/30 p-6 backdrop-blur shadow-2xl">
      <div className="flex items-center gap-3 border-b border-slate-800/80 pb-4 mb-4">
        ...
        <div>
          <h2 className="text-lg font-extrabold text-slate-100">{t('radio.title')}</h2>
          <p className="text-xs text-slate-400 mt-0.5">{t('radio.desc')}</p>
        </div>
      </div>

      {/* Driver Input Selector */}
      <div className="flex flex-col gap-2 mb-6 sm:w-1/3">
        <label className="text-xxs font-bold text-slate-400 uppercase tracking-wider">{t('radio.driver')}</label>
        <input
          type="number"
          value={driverNumber}
